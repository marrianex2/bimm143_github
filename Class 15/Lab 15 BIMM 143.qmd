---
title: "Lab 15: Pertussis and the CMI-PB project"
author: "Marriane Allahwerdi (A16902759)"
format: pdf
---
##  Background

Pertussis, a.k.a whooping cough, is a highly infectious disease caused by bacteria *B. Pertussis*
...
We went from ~200,000 cases pre wP to ~1000 cases in 1976.The numbers started to rise again after the introduction of the aP vaccine. 
...
There was a gradual rise after it the aP vaccine was introduced in 1995. We start to see a big increase in 2004 to ~26,000 cases. This could be due to bacterial evolution, vaccination hesitancy, waning immunity in children. 
There is a ~10 year lag from the aP roll out to increasing case numbers. This holds true of other countries like Japan, UK, etc. 

**Key Question**: why does the aP vaccine induced immunity wane faster than that of the wP vaccine? 

## CMI-PB 

The CMI-BP (Computational Models of Immunity Pertussis Boost) makes available lots of data about the immune response to Pertussis booster vaccination. 

Critically it tracks the wP and aP individuals over time to see how their immune response changes. 
...
Let's get mpre data from CMI-PB, this time about the specimens collected. 
...
Now we can join (merge) these two tables `subject` and `specimen` to make on new `meta` table with the combined data. 
```{r}
library(dplyr)
...
One more joined to do here of `meta` and `abdata` to associate all the metadata about the individual and their race, biological sex and infancy vaccination status together with Antibody levels...
...
How many isotypes are covered in this dataset
...
table(ab$antigen)
```
Let's focus in on IgG - one of the main antibodies responsive to bacteria or viral infections
...
Make a first plot of MFI (Mean Fluorensence Intensity - a measure of how much is detected) for each antigen.
...
```{r}
ggplot(igg) + aes(MFI_normalised, antigen) + geom_boxplot() + facet_wrap(~visit)
```
...
Looks like we don't have data yet for all subjects in terms of visits 8 on-wards. So lets exclude these
